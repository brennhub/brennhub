...
### 결정론·정직성 — 규칙·확률·AI의 경계 (tag-it에서 정립)

- **세상은 흑백이 아니다 — 정직한 설계**: 시스템이 모르는 것을 아는 척하지 않는다. 불확실성을
  제거/보존 이분법으로 자르지 말고 확률·정도로 사용자에게 정직하게 노출. **확정 못 하는 건 확정
  안 한다.**
- **사전·자료는 규칙을 발견·검증하는 토대** (버릴 대상이 아니라): 규칙을 직관이 아니라 데이터로
  세우고 검증한다 (충돌 수 등 실측 → 안전한 규칙만 채택).
- **결정론의 한계 = AI의 영역**: 문맥 의존 판별(동음이의 등)은 알고리즘으로 100% 불가. AI 도입
  전까지 **자체 결정론 시스템을 최대로 단단히** → AI는 보완 안전망으로(호출 최소·비용 균형). 단단한
  자체 시스템은 유사 서비스의 **진입장벽 자산**. ([§3] "결정론을 AI보다 우선"의 경계 정의.)
- **가중치·규칙은 잠정 추정이지 진리가 아니다**: "우리 데이터 안의 추정"일 뿐 언어/세상의 진실이
  아니다 — **보수적으로, 큰 차감(파괴) 금지, 항상 조정 가능하게**(코드 수정 없이 값만).
- **파괴보다 후순위**: 오탐 제거(정답을 죽임)는 노이즈 잔존보다 나쁘다(검색 인덱서의 진리). 단
  결과를 직접 보여주는 UI는 첫 화면 정밀도도 중요 → **"삭제 대신 후순위 정렬"로 균형**(질 vs 양).
...
- **기존 도구 새 task 시작**: `git checkout feat/<tool>` → **`git branch`로 현재 브랜치 확인** → `git merge main` (main 최신 변경 sync). ⚠️ 브랜치 확인은 필수 — 과거 main/dev에 직접 commit한 혼동 사고 방지.
- 작업 + commit.
