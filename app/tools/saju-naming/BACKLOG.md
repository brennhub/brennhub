...
- ◐ 상용도 티어 + 작명 부적합 가드 (39-C, 2026-05-29) — `frequency` 컬럼을 상용도 프록시(Unihan KS X 1001/1002/확장 + 교육용 1,800)로 populate(007 migration) + `recommendNames` cap-skip 버퍼(007 freq-DESC 풀로 상용 char2 우선) + `freqSum` 동점 tiebreak + `name-exclude` 작명 부적합 의미군(숫자·기능어). prod 蘇玟刁 류(벽자·숫자·기능어) 해소. dev HTTP 회귀 ✅(차단대상 0·null 0·**0/40 503**·p50 214 p95 313ms). 상세 `docs/learnings/2026-05-29-saju-naming-39c-frequency-tier.md`.
  - **반영 상태 (2026-06-04 기준)**: 39-C 코드 커밋(`ff7b5b8` 외)은 그 후 origin/main에 머지되어 **prod에 반영됨**. 007 migration `dev preview apply 완료 / prod D1 apply 상태는 별도 확인 항목` (코드는 prod지만 데이터 적용 시점이 분리). 2026-05-29 시점의 "prod 보류" 결정은 그 뒤 번복됨 — 추천 품질 잔여 이슈(蘇황료 류 char2 반복)는 BACKLOG의 "작명 품질 본질"·char2 best-by-score 항목으로 추적.
- [ ] **char2 best-by-score (39-C 보류 — CPU 한계)** — 첫 글자별 최고점 char2 선택은 n=2 전 조합(POOL 500²=25만)을 `evaluateSoundOhaeng`로 평가해야 해 Workers CPU 503(dev 2회 회귀: 무제한 17/20, 상위40 3/20). cap-skip(수백 조합)으로 복귀, 007 freq-DESC 풀이 상용 char2를 대체 보장. 재시도 경로: `evaluateSoundOhaeng` 경량화(sung→c1 부분 결과 메모이즈 + c1→c2만 증분) 또는 char2 후보 사전 점수 프록시. 그 후 best-by-score로 char2 변별 향상.
