...
- ◐ 상용도 티어 + 작명 부적합 가드 (39-C, 2026-05-29) — `frequency` 컬럼을 상용도 프록시(Unihan KS X 1001/1002/확장 + 교육용 1,800)로 populate(007 migration, dev apply 완료) + `recommendNames` cap-skip 버퍼(007 freq-DESC 풀로 상용 char2 우선) + `freqSum` 동점 tiebreak + `name-exclude` 작명 부적합 의미군(숫자·기능어). prod 蘇玟刁 류(벽자·숫자·기능어) 해소. dev HTTP 회귀 ✅(차단대상 0·null 0·p50 178ms). 상세 `docs/learnings/2026-05-29-saju-naming-39c-frequency-tier.md`.
