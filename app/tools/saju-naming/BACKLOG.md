...
- ◐ 상용도 티어 + 작명 부적합 가드 (39-C, 2026-05-29) — `frequency` 컬럼을 상용도 프록시(Unihan KS X 1001/1002/확장 + 교육용 1,800)로 populate(007 migration, **dev preview apply 완료 / prod 미적용**) + `recommendNames` cap-skip 버퍼(007 freq-DESC 풀로 상용 char2 우선) + `freqSum` 동점 tiebreak + `name-exclude` 작명 부적합 의미군(숫자·기능어). prod 蘇玟刁 류(벽자·숫자·기능어) 해소. dev HTTP 회귀 ✅(차단대상 0·null 0·**0/40 503**·p50 214 p95 313ms). 상세 `docs/learnings/2026-05-29-saju-naming-39c-frequency-tier.md`.
  - **⚠️ prod 보류 결정 (2026-05-29)**: dev 검증은 통과(0 503·벽자/기능어 제거)했으나, 추천 체감 품질이 여전히 비-작명(char2 반복 了/夕/入·점수 포화) → 사용자 눈엔 蘇玟刁→蘇황료 횡이동. **main 머지 보류**, Task 45(AI 어감)/작명-특화 빈도로 실제 좋은 이름 선별 후 prod 진입. dev/feat은 인프라로 보존. **prod D1에 007 적용 금지**(prod 코드 미반영 — 데이터/코드 불일치 방지).
