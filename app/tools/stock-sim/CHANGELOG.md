...
- **분할매도 회차별 누적 손익 + 총 투자금 + 보유주식수 입력 raw** — ① 회차별 매도 테이블 맨 오른쪽에 "누적 손익" 컬럼 추가(회차별 실현손익의 running sum, 마지막 행 = 요약 "실현 손익"과 일치). 색은 실현 손익과 동일(KR/US 컨벤션). CSV에도 포함. ② 결과 요약 맨 위에 "총 투자금" 행 추가(= 보유주식수 × 평단가, 통화 토글 적용). ③ 보유 주식수 입력란의 천단위 쉼표 포맷팅 제거 — raw 숫자 표시. 요약·테이블 출력의 쉼표는 유지(입력만 변경). 매도가·실현손익·세금 계산 동작 무변경.
