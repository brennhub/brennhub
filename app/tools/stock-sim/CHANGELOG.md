...
## 2026-05-21
- **분할매도 시뮬레이션 추가** — 분할매수 탭에 "분할 매도 시뮬레이션" 토글(기본 OFF). ON 시 매도 횟수 / 상승율 입력으로 보유 포지션을 회차별 매도하는 가상 시나리오 계산. 매수와 동일 가중치(Martingale 2x / 0~100 균형)를 공유, 매도 1회차 가격 = 평단가 × (1 + 상승율%). 매도 ON 시 목표 수익률 입력 비활성 + 매수 테이블 수익 / 목표가 컬럼 숨김. 평균 매도가 / 실현 손익 요약, 회차별 매도 테이블(`dca-down-sell-detail.tsx`, 회차 완료 마킹 + CSV). 보유 주식수는 최대잔여 정수 배분으로 회차 분배(합 정확 일치). 세금은 전체 실현손익에 단일 세율. localStorage additive — 기존 사용자 데이터 호환.
